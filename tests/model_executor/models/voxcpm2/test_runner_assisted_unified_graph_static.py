...
def test_ar_runner_exposes_runner_assisted_full_metadata_hook():
    ...
    assert "needs_runner_assisted_full_attention_metadata" in source
    assert "is_runner_assisted_full_attention_metadata_capture" in source
    assert "set_runner_assisted_full_attention_metadata_context" in source
    assert "get_runner_assisted_full_attention_metadata_padded_num_reqs" in source
    ...
    assert "cudagraph_runtime_mode=(CUDAGraphMode.FULLifrunner_assisted_full_attnelsecudagraph_mode)" in compact_source
    ...
    padding_source = source[
        source.index("runner_assisted_full_attn = self._needs_runner_assisted_full_attention_metadata") :
    ]
    padding_source = padding_source[: padding_source.index("runner_assisted_full_attn_capture =")]
    assert "_get_runner_assisted_full_attention_metadata_padded_num_reqs" in padding_source
...
def test_voxcpm2_batch_unified_graph_requires_runner_metadata_marker():
    ...
    needs_source = source[source.index("def needs_runner_assisted_full_attention_metadata") :]
