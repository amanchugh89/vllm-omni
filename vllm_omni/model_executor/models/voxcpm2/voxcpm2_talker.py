...
class VoxCPM2TalkerForConditionalGeneration(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self._enable_unified_decode_graph = use_cuda_graph and self._runtime_config.enable_unified_decode_graph
        ...
        self._enable_decode_tail_graph = use_cuda_graph and self._runtime_config.enable_decode_tail_graph
    ...
    @staticmethod
    def _compile_without_inductor_cudagraphs(
        ...
        if cfg.enable_loc_dit_layer_nvtx:
            ...
            try:
                if cfg.enable_loc_dit_reduce_overhead_no_cg:
                    tts.feat_decoder.estimator = self._compile_without_inductor_cudagraphs(
                        estimator,
    ...
    def _parse_graph_pre_capture_sizes(
        ...
        *,
        ...
        sizes = {
            ...
            if 0 < size <= max_size
            and size in self._unified_graph_bucket_sizes
    ...
    def needs_runner_assisted_full_attention_metadata(
        self,
        *,
        req_ids: list[str],
        num_reqs: int,
        ...
        del num_tokens_unpadded, cudagraph_mode, batch_descriptor
    ...
    def set_runner_assisted_full_attention_metadata_context(
        ...
        *,
        ...
        del num_reqs_padded, num_tokens_padded, cudagraph_mode, batch_descriptor
        del attn_metadata, slot_mapping
    ...
    def is_runner_assisted_full_attention_metadata_capture(
        self,
        *,
        num_reqs: int,
        num_reqs_padded: int,
        ...
        del num_reqs_padded, num_tokens_padded, batch_descriptor
        return (
            self._runtime_config.enable_runner_assisted_unified_decode_graph
            and num_reqs > 1
            and (
                (bucket_size := self._select_unified_graph_bucket_size(num_reqs)) is not None
                and bucket_size not in self._unified_graphs
            )
        )

    def get_runner_assisted_full_attention_metadata_padded_num_reqs(
        self,
        *,
        num_reqs: int,
        num_reqs_padded: int,
        ...
        return int(bucket_size or num_reqs)
